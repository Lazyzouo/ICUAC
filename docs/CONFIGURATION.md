...
Chinese mode reads `messages` from `config.yml`, preserving legacy customization. English mode reads `plugins/ICUAC/lang/en_US.yml` and falls back to the config message when a key is missing. Leading whitespace is removed from every player-visible line at send time, including messages from an existing configuration, so help titles, command entries, warnings, and other plugin text begin at the left edge. Legacy `&` colors and `&#RRGGBB` colors are parsed, but player-visible foreground colors are replaced by the continuous `#00D2FF` → `#3A7BD5` → `#F2C94C` gradient and all readable text is forced bold. Console output keeps its configured colors and alignment; pure decorative divider lines stay unbold.
...
中文模式读取 `config.yml` 的 `messages`，保持旧版自定义兼容；英文模式读取 `plugins/ICUAC/lang/en_US.yml`，缺少键时回退到配置消息。所有玩家可见消息在发送时都会移除行首空白，现有配置同样生效，使帮助标题、命令项、警告及其他插件文字均从左边缘开始。插件仍会解析 `&` 与 `&#RRGGBB` 颜色，但玩家可见文字的前景色会统一替换为连续的 `#00D2FF` → `#3A7BD5` → `#F2C94C` 渐变，且全部可读文字强制粗体。后台保留配置颜色与对齐方式，纯装饰分割线保持非粗体。
