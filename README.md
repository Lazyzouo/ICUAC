...
| --- | --- | --- |
| `ICUAC-2.1.1-zh.cn.jar` | Simplified Chinese (`zh_CN`) | Chinese server administrators |
| `ICUAC-2.1.1-en.us.jar` | English (`en_US`) | English server administrators |
...
| Console presentation | Uses a consistent ICUAC prefix, bold text, a bilingual startup banner centered on a `✧` divider axis with CJK-aware widths, detected Paper/Folia platform, and status-colored updater notices. |
...
- `ICUAC-2.1.1-zh.cn.jar`：默认简体中文。
- `ICUAC-2.1.1-en.us.jar`：默认英文。
...
| 后台显示 | 使用统一 ICUAC 前缀、全部粗体文字、以 `✧` 分割线为中轴并按中英文宽度居中的双语启动横幅、实际 Paper/Folia 核心识别及按状态着色的更新通知。 |
